...
def testPath(top, testBlock, testRoot, testFile, onreaderror = None, mode = 'rb'):
    for version, user_version, f, root_blocks in NifFormat.walkFile(top, onerror = onreaderror, verbose = 1, mode = mode):
        if testRoot:
            for root in root_blocks:
                testRoot(root, verbose = 1)
        if testBlock:
            # calculate blocks beforehand as tree may change after each test
...
Usage:  Specify a nif file or folder as first argument.
        NifTester will look for a file called "testers/hacking/<tester>.py",
        "testers/validate/<tester>.py", or "testers/surgery/<tester>.py" and
        use it for testing the specified
        ...
        The difference between validate and surgery tests is that the surgery test
        modifies the original nif file.
...
"""
    ...
    testPath(top, testBlock, testRoot, testFile, onreaderror, mode)
